...
namespace AlgorithmAndDS.LeetCode.String
{
    public class TextJustification
    {
        ...
        public static string[] FullJustify(string[] words, int maxWidth)
        {
            ...
            List<string> listOfStr = new();
            int totalWhiteSpace = maxWidth;
            int j = 0;
            int width = 0;
            for (int i = 0; i < words.Length; i++)
            {
                if (words[i].Length + width < maxWidth)
                {
                    ...
                    totalWhiteSpace -= words[i].Length;
                }
                else
                {
                    var diff = (i - j - 1);
                    var space = (totalWhiteSpace /(diff==0?1:diff) );
                    while (j < i)
                    {
                        sb.Append(words[j]);
                        sb.Append(' ',space);
                    ...
                    listOfStr.Add(sb.ToString());

                    //reset width ,reset j,
                    width = words[i].Length;
                    //j=i;
                    totalWhiteSpace = maxWidth-width;
                    sb.Clear();
                }
